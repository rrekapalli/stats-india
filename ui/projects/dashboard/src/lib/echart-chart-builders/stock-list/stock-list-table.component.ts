...
export class StockListTableComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  private primeReloadScheduled = false;
  ...
  private updateStocksFromWidget(): void {
    ...
    if (this.widget?.data?.stocks) {
      ...
      if (rawStocks.length > 0) {
        ...
        if (this.widget.data.footerStats && this.widget.data.footerStats.totalCount > 0) {
  ...
  get footerAvgAbsChange(): number | null {
    return this.averageAbsMetric('priceChange', this.filteredStocks);
  }

  get footerAvgAbsPercentChange(): number | null {
    return this.averageAbsMetric('percentChange', this.filteredStocks);
  }

  private averageAbsMetric(
    field: 'priceChange' | 'percentChange',
    list: StockListData[]
  ): number | null {
    if (!list.length) {
      return null;
  ...
  private loadPrimeFilteredList(): void {
    ...
    if (this.isLoadingPrimeFilteredList) {
      this.primeReloadScheduled = true;
      return;
    }
    ...
    const loadPage = () => {
      ...
      this.agGridEnv!.getFilteredInstrumentsPost(request).subscribe({
        ...
        error: () => {
          ...
          if (this.primeReloadScheduled) {
            this.primeReloadScheduled = false;
            this.loadPrimeFilteredList();
          }
          this.cdr.markForCheck();
        }
